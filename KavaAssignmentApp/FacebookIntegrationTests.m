...
- (void) testFacebookLoginUIWhenDataDownloaded
{
    [[[self.controllerMock stub] andReturnValue:OCMOCK_VALUE(YES)] facebookDataFetched];
    [[[self.controllerMock stub] ignoringNonObjectArgs] setFacebookDataFetched:YES];
    
    [[[self.controllerMock reject] ignoringNonObjectArgs] presentViewController:[OCMArg checkWithBlock:^BOOL(id object){
        return [object isKindOfClass:[FBUserSettingsViewController class]];
    }] animated:NO completion:OCMOCK_ANY];
    [self.controller viewDidLoad];
    [self.controllerMock verify];
